...
> 크레딧 차감 정책은 미정. 현재 유력한 방식은 MCP 서버가 실행 후 크레딧 서버를 직접 호출하는 구조이며, 이 경우 관리 서버는 크레딧 차감에 개입하지 않는다.
...
> 등록 완료 후 `tools/list` · `resources/list`를 자동 수집한다. `type=WEBAPP`이면 수집을 생략하고 바로 `ACTIVE`로 전환한다.
