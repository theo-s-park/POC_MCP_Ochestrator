---
description: MCP 서버를 생성하거나 기존 서버에 MCP 어댑터를 추가할 때 사용하는 스킬. PO MCP 관리 서버 연동에 필요한 `/health`, `/mcp` 엔드포인트 구현과 배포 후 등록 스크립트까지 안내한다.
---

# MCP 서버 생성 스킬 — PO MCP 관리 서버 연동
...
- "MCP 서버 추가해줘"
- "이 서버를 MCP로 등록하고 싶어"
- "PO MCP 관리 서버에 연동하고 싶어"

## 스킬 개요

PO MCP 관리 서버는 MCP 서버를 등록·관리·제공하는 레지스트리다.
이 스킬은 어떤 언어/프레임워크로 구현하든 PO MCP 관리 서버와 연동 가능한 MCP 서버를 만드는 데 필요한 요건을 안내한다.
...
- **사용자 목표**: 새로운 기능을 MCP 서버로 만들어 PO MCP 관리 서버에 등록
...
MCP 관리 서버가 60초마다 폴링하여 서버 상태를 판단한다. 3회 연속 실패 시 INACTIVE 전환.
...
MCP 관리 서버가 등록 시점에 1회 호출한다. 서버가 제공하는 도구 목록을 반환한다.
...
WEB/WAS 클라이언트가 MCP 관리 서버를 경유하지 않고 **직접** 호출한다.
...
MCP 관리 서버가 `tools/list` 수집 시 `inputSchema`를 함께 저장하여 WEB/WAS에 그대로 전달한다.
